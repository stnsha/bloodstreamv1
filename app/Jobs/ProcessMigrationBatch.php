...
class ProcessMigrationBatch implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            // Get all pending item IDs (optimized - was loading full models)
            $itemIds = $batch->items()->where('status', MigrationBatchItem::STATUS_PENDING)->pluck('id');
            ...
            foreach ($itemIds as $index => $itemId) {
                ...
                ProcessMigrationReport::dispatch($itemId)
                    ->onQueue('migration')  // Use migration queue
                    ->delay(now()->addSeconds($delay));

                // Log every 10th dispatch
                if ($index % 10 == 0 || $index == $itemIds->count() - 1) {
