...
php -d memory_limit=%PHP_MEMORY_LIMIT% -d max_execution_time=900 artisan queue:work database --queue=ai-webhooks,ai-reviews,migration --timeout=300 --memory=1024 --max-jobs=50 --max-time=600 --sleep=3 --tries=3 --rest=1 2>> storage\logs\queue_worker.log
